...
The build consumes a reproducible `git archive` of that commit and verifies its
SHA-256 before extraction, so the build does not depend on a live GitHub fetch.
