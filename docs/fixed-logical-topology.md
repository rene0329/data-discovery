...
超过 `app.network-topology.stale-after-seconds` 未更新的成功记录，在读取时标记 `STALE`。默认 93600 秒（26 小时），匹配当前每日一次的限流量探测，并留出两小时的调度、重试和上报余量。探测周期与该有效期必须一起调整。每日测速不提供实时链路故障检测；探测明确失败仍立即标为 inactive，失败、未知、过期的边不参加调度。节点停用、离线或观测过期时，也不能作为路径的起点、终点或中间节点。
